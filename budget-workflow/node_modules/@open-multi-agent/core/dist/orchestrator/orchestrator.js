...
function computeRunMetrics(tasks) {
    ...
    let minTaskDurationMs;
    let maxTaskDurationMs;
...
function estimateIncrementalCost(usage, context, estimateCost) {
    const cost = estimateCost(usage, context);
    if (!Number.isFinite(cost) || cost < 0) {
        throw new Error(`Cost estimator returned invalid cost for agent "${context.agentName}": ${String(cost)}`);
    }
    return cost;
}
function applyBudgetAccounting(params) {
    ...
    const cumulativeCost = params.estimateCost
        ...
        : params.currentCost;
    if (params.maxTokenBudget !== undefined
        && totalTokens(cumulativeUsage) > params.maxTokenBudget) {
        return {
            cumulativeUsage,
            cumulativeCost,
            exceeded: new TokenBudgetExceededError(params.costContext.agentName, totalTokens(cumulativeUsage), params.maxTokenBudget),
        };
    }
    if (params.maxCostBudget !== undefined
        && params.estimateCost !== undefined
        && cumulativeCost > params.maxCostBudget) {
        return {
            cumulativeUsage,
            cumulativeCost,
            exceeded: new CostBudgetExceededError(params.costContext.agentName, cumulativeCost, params.maxCostBudget),
        };
    }
    return { cumulativeUsage, cumulativeCost };
}
function emitBudgetExceeded(config, error, agent, task) {
    config.onProgress?.({
...
function recordRunUsage(ctx, usage, costContext, agent = costContext.agentName, task) {
 ...
 */
export async function executeWithRetry(run, task, onRetry, delayFn = abortableDelay, opts) {
    ...
    const backoffSleep = async (attempt) => {
...
function buildTaskAgentTeamInfo(ctx, taskId, traceBase, delegationDepth, delegationChain) {
    ...
    const runDelegatedAgent = async (targetAgent, prompt) => {
        ...
        const nestedTeam = buildTaskAgentTeamInfo(ctx, taskId, childTraceBase, delegationDepth + 1, [...delegationChain, targetAgent]);
        const childOpts = {
            ...childTraceBase,
            team: nestedTeam,
        };
...
async function executeQueue(queue, ctx) {
    ...
    while (true) {
        ...
        const dispatchPromises = pending.map(async (task) => {
            ...
            }, undefined, { abortSignal: ctx.abortSignal });
...
async function runConsensusCore(params) {
    ...
    const runEphemeral = async (config, text) => {
...
export class OpenMultiAgent {
    ...
    async runCoordinatorSynthesis(team, queue, goal, coordinatorBaseConfig, opts) {
        ...
        if (opts.maxCostBudget !== undefined
        ...
        const synthesisConfig = withModelRoute(coordinatorBaseConfig, routeMatches(opts.modelRouting, { phase: 'synthesis', agent: 'coordinator' }));
