...
export class TaskQueue {
    ...
    static taskFromSnapshot(snapshot) {
        return {
            id: snapshot.id,
    ...
    static parseSnapshotDate(value) {
        const date = new Date(value);
        return Number.isNaN(date.getTime()) ? new Date() : date;
    }
    static resetRestoredInProgress(tasks) {
        const initial = tasks.map((task) => task.status === 'in_progress'
            ? { ...task, status: 'pending', updatedAt: new Date() }
            : task);
        const taskById = new Map(initial.map((task) => [task.id, task]));
