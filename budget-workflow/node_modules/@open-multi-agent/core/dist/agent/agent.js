...
export class Agent {
    ...
    constructor(config, toolRegistry, toolExecutor) {
        ...
        if (config.model === undefined && config.backend === undefined) {
            throw new Error(`Agent "${config.name}" has no model. Set 'model' in its config, or a 'backend', ` +
    ...
    async getBackend() {
        ...
        this.backend = new AgentRunner(adapter, this._toolRegistry, this._toolExecutor, runnerOptions);
        return this.backend;
    }
    /**
     ...
     */
    async createExternalBackend(backend) {
        switch (backend.kind) {
            case 'acp': {
            ...
            default:
                throw new Error(`Agent "${this.name}": unknown backend kind "${backend.kind}".`);
        }
    }
    // -------------------------------------------------------------------------
    ...
    async executeRun(messages, callerOptions) {
        ...
        let effectiveTraceOptions = callerOptions;
        try {
            // --- beforeRun hook ---
            ...
            if (this.config.outputSchema) {
                let validated = await this.validateStructuredOutput(messages, result, backend, runOptions);
                // --- afterRun hook ---
                if (this.config.afterRun) {
     ...
     */
    async validateStructuredOutput(originalMessages, result, backend, runOptions) {
