...
const SENSITIVE_NAME_PATTERN = /(?:api[_-]?key|apiKey|secret|password|passwd|pwd|private[_-]?key|privateKey|access[_-]?key|accessKey|accessToken|refreshToken|idToken|githubToken|authorization|auth[_-]?token|authToken|cookie|session|credential|bearer|^token$|[_-]token$|^token[_-])/i;
...
const QUOTED_ASSIGNMENT_PATTERN = /(["']?)([A-Za-z_][A-Za-z0-9_.-]*)(\1?\s*[:=]\s*)(["'])([^]*?)\4/g;
...
const UNQUOTED_ASSIGNMENT_PATTERN = /(["']?)([A-Za-z_][A-Za-z0-9_.-]*)(\1?\s*[:=]\s*)([^"'\s,;}\]\n\r][^,;}\]\n\r]*)/g;
const TOKEN_LITERAL_PATTERNS = [
 ...
 */
export function redactSensitiveText(text, extraPatterns = []) {
    if (text.length === 0)
        return text;
    let redacted = text.replace(/-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----/g, REDACTED);
    redacted = redacted.replace(/\b(Authorization\s*:\s*)(?:Bearer\s+)?[^\n\r,;}]+/gi, `$1${REDACTED}`);
    redacted = redacted.replace(/\bBearer\s+[A-Za-z0-9._~+/=-]{8,}/gi, `Bearer ${REDACTED}`);
    ...
    redacted = redacted.replace(QUOTED_ASSIGNMENT_PATTERN, (match, keyQuote, key, separator, valueQuote, _value) => {
    ...
    redacted = redacted.replace(UNQUOTED_ASSIGNMENT_PATTERN, (match, keyQuote, key, separator, value) => {
        if (!isSensitiveName(key))
            return match;
        ...
        if (value === '[redacted' || value.startsWith('[redacted'))
            return match;
 ...
 */
export function redactSensitiveObject(value, extraPatterns = []) {
    return redactValue(value, extraPatterns);
}
function redactValue(value, extraPatterns, key) {
