 ...
 */
export function toOpenAIMessages(messages, outboundOptions) {
    const result = [];
    ...
    const conversationHasToolUse = messages.some((m) => m.content.some((b) => b.type === 'tool_use'));
 ...
 */
function toOpenAIAssistantMessage(msg, outboundOptions, conversationHasToolUse = false) {
    const toolCalls = [];
    const textParts = [];
    const pendingThinkingParts = [];
 ...
 */
export function fromOpenAICompletion(completion, knownToolNames, provenance) {
    ...
    if (reasoningText.length > 0) {
        const reasoningBlock = provenance !== undefined
 ...
 */
export function buildOpenAIMessageList(messages, systemPrompt, outboundOptions) {
    const result = [];
