...
const RESERVED_STORE_PREFIXES = ['__oma_checkpoint__/'];
/**
...
export class SharedMemory {
     ...
     */
    static async fromSnapshot(snapshot, store) {
     ...
     */
    async write(agentName, key, value, metadata, options) {
     ...
     */
    async writeExpiring(agentName, key, value, ttlTurns, metadata, options) {
     ...
     */
    async read(key) {
        if (SharedMemory.isReservedStoreKey(key))
            return null;
    ...
    async getSummary(filter) {
        ...
        for (const [agent, entries] of byAgent) {
            ...
            for (const { localKey, value } of entries) {
                ...
                const truncated = displayValue.length > 200 ? `${displayValue.slice(0, 197)}…` : displayValue;
    ...
    static formatValueForSummary(value) {
        return typeof value === 'string' ? value : JSON.stringify(value);
    }
    static assertJsonSerializable(value) {
        SharedMemory.assertJsonSerializableValue(value, new WeakSet(), '<root>');
    }
    static assertJsonSerializableValue(value, seen, path) {
        if (value === null)
            return;
