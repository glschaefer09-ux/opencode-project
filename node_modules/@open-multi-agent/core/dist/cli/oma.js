...
// ---------------------------------------------------------------------------
export const PROVIDER_REFERENCE = [
...
function asTeamConfig(v, label) {
    if (!isObject(v))
        throw new OmaValidationError(`${label}: expected a JSON object`);
    const name = v['name'];
    const agents = v['agents'];
    if (typeof name !== 'string' || !name)
        throw new OmaValidationError(`${label}.name: non-empty string required`);
    if (!Array.isArray(agents) || agents.length === 0) {
        throw new OmaValidationError(`${label}.agents: non-empty array required`);
    }
    for (const a of agents) {
        if (!isObject(a))
            throw new OmaValidationError(`${label}.agents[]: each agent must be an object`);
        if (typeof a['name'] !== 'string' || !a['name'])
            throw new OmaValidationError(`agent.name required`);
    ...
    if ('sharedMemoryStore' in v) {
        throw new OmaValidationError(`${label}.sharedMemoryStore: SDK-only; cannot be set from JSON config. ` +
...
function asOrchestratorPartial(v, label) {
    if (!isObject(v))
        throw new OmaValidationError(`${label}: expected a JSON object`);
    return v;
}
function asCoordinatorPartial(v, label) {
    if (!isObject(v))
        throw new OmaValidationError(`${label}: expected a JSON object`);
    return v;
}
function asTaskSpecs(v, label) {
    if (!Array.isArray(v))
        throw new OmaValidationError(`${label}: expected a JSON array`);
    const out = [];
    let i = 0;
    for (const item of v) {
        if (!isObject(item))
            throw new OmaValidationError(`${label}[${i}]: object expected`);
        ...
        if (Array.isArray(item['dependsOn'])) {
            row.dependsOn = item['dependsOn'].filter((x) => typeof x === 'string');
        }
        if (item['memoryScope'] === 'all' || item['memoryScope'] === 'dependencies') {
            row.memoryScope = item['memoryScope'];
        }
        if (typeof item['maxRetries'] === 'number')
            row.maxRetries = item['maxRetries'];
        if (typeof item['retryDelayMs'] === 'number')
            row.retryDelayMs = item['retryDelayMs'];
        if (typeof item['retryBackoff'] === 'number')
            row.retryBackoff = item['retryBackoff'];
        out.push(row);
        i++;
    }
    return out;
}
export function serializeAgentResult(r, includeMessages) {
    const base = {
        success: r.success,
        output: r.output,
...
export function serializeTeamRunResult(result, opts) {
    const agentResults = {};
...
async function cmdProvider(sub, arg, pretty) {
...
function mergeOrchestrator(base, ...partials) {
    let o = { ...base };
...
async function writeRunTeamDashboardFile(html) {
...
async function main() {
    ...
    const jsonOpts = { pretty, includeMessages };
    try {
        if (cmd === 'run') {
            ...
            if (dashboard) {
                ...
                try {
                    ...
                    process.stderr.write(`oma: failed to write runTeam dashboard: ${err instanceof Error ? err.message : String(err)}\n`);
                }
            }
            await orchestrator.shutdown();
            const payload = { command: 'run', ...serializeTeamRunResult(result, jsonOpts) };
        ...
        if (cmd === 'task') {
            ...
            await orchestrator.shutdown();
            const payload = { command: 'task', ...serializeTeamRunResult(result, jsonOpts) };
            printJson(payload, pretty);
            return result.success ? EXIT.SUCCESS : EXIT.RUN_FAILED;
        }
...
function classifyCliError(e, message) {
    if (e instanceof OmaValidationError)
        return { kind: 'validation', exit: EXIT.USAGE };
    if (message.includes('Invalid JSON'))
        return { kind: 'validation', exit: EXIT.USAGE };
    if (message.includes('ENOENT') || message.includes('EACCES'))
        return { kind: 'io', exit: EXIT.USAGE };
